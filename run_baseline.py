...
CONFIG_PATH = {
    'textual': 'config/textual_config.yaml',
    'visual': 'config/visual_config.yaml',
    'multimodal': 'config/multimodal_config.yaml',
}
...
if GLOBAL_ARGS.framework_name == 'textual':
    from models import TextualTransformerModel as Model
elif GLOBAL_ARGS.framework_name == 'visual' and not GLOBAL_ARGS.model_name == 'resnet':
    from models import VisualTransformerModel as Model
elif GLOBAL_ARGS.framework_name == 'visual' and GLOBAL_ARGS.model_name == 'resnet':
    from models import ResNetModel as Model
elif GLOBAL_ARGS.framework_name == 'multimodal' and GLOBAL_ARGS.model_name == 'clip':
    from models import ClipModel as Model
elif GLOBAL_ARGS.framework_name == 'multimodal' and GLOBAL_ARGS.model_name == 'vilt':
    from models import ViltModel as Model
elif GLOBAL_ARGS.framework_name == 'multimodal':
    from models import MultiModalTransformerModel as Model
else:
...
def main(args=None):
    ...
    for target_name in target_names:
        ...
        elif GLOBAL_ARGS.framework_name == 'visual':
            tokenizer = AutoImageProcessor.from_pretrained(args.transformer_tokenizer_name)
        elif GLOBAL_ARGS.framework_name == 'multimodal':
            if GLOBAL_ARGS.model_name == 'clip' or GLOBAL_ARGS.model_name == 'vilt':
                multimodal_processor = AutoProcessor.from_pretrained(args.multimodal_processor_name, model_max_length=args.max_tokenization_length)
                tokenizer = (multimodal_processor.tokenizer, multimodal_processor.image_processor)
            else:
                textual_tokenizer = AutoTokenizer.from_pretrained(args.textual_transformer_tokenizer_name, model_max_length=args.max_tokenization_length)
